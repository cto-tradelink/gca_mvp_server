...
#         obj["support_business_apply_end_ymdt"] = str(support_business.support_business_apply_end_ymdt).split(" ")[0]
...
#         FavoriteLog.objects.get(user_id=user_id, support_business_id=support_business.id).delete()
#         result="remove"
#     else:
...
#
#
# # ------------(스타트업) 유저가 본인이 하트 누른'관심 지원사업'을 관심지원사업 리스트에 데이터 가져오기 : 관심지원사업 리스트
# # --------[관심 지원사업 리스트 가져오기]-------
#
#
#
...
#
#     if Startup.objects.get(id=request.GET.get("startup_id")) not in AdditionalUserInfo.objects.get(id=user_id).favorite_startup.all():
#         AdditionalUserInfo.objects.get(id=user_id).favorite_startup.add(Startup.objects.get(id=request.GET.get("startup_id")))
#         FavoriteLog(startup=Startup.objects.get(id=request.GET.get("startup_id")) , user_id = user_id ).save()
#         result ="add"
#     else:
#         AdditionalUserInfo.objects.get(id=user_id).favorite_startup.remove(Startup.objects.get(id=request.GET.get("startup_id")))
#         FavoriteLog.objects.get(startup=Startup.objects.get(id=request.GET.get("startup_id")), user_id=user_id).delete()
#         result="remove"
#     return JsonResponse({"result":result})
#
...
#
#
# # --------[5. 지원서 정보 등록 > 스타트업 정보 업데이트 ]-----------------------------------------------------------------------------
# # ------------(스타트업) 지원서에 다섯번째 페이지 작성한 것을 기준으로 스타트업 정보를 업데이트
# #--- 다른 파일 업로드 하기
#
# def vue_update_startup_with_application_5(request):
#     if gca_check_session(request)== False:
#         return HttpResponse("{}")
#     rjd = json.loads(request.POST.get("json_data"))
#     app = Appliance.objects.get(id=rjd["id"])
#     app.company_intro = rjd["company_intro"]
#     for his in ApplianceHistory.objects.filter(appliance=app):
#         his.delete()
#     for h in rjd["history"]:
#         try:
#             if h["id"]:
#                 history = ApplianceHistory.objects.get(id=h["id"])
#             else:
#                 history = ApplianceHistory()
#         except:
#             history = History()
#         history.company_history_year = rjd["company_history_year"]
#         history.company_history_month = rjd["company_history_month"]
#         history.company_history_content = rjd["company_history_content"]
#         history.save()
#     return JsonResponse({"result":"ok"})
#
...
#     result={}
#     result["favorite"] = []
#     for favorite_list in AdditionalUserInfo.objects.get(id=request.POST.get("id")).favorite.all():
#         temp={}
#         temp["support_business_name"] = favorite_list.support_business_name
#         temp["favorite"] = len(favorite_list.additionaluserinfo_set.all())
#         if favorite_list.support_business_recruit_size == "" or   favorite_list.support_business_recruit_size== "0" :
#
#             temp["comp"] =  str(len(Appliance.objects.all().filter(support_business=favorite_list).filter(is_submit=True))) +" : 1"
#         else:
#             # temp["comp"] = len(Appliance.objects.all().filter(support_business=inte)) / int(inte.recruit_size)
...
#
#
#         temp["date"] = str(favorite_list.support_business_apply_end_ymdt).split(" ")[0]
#         temp["start"] = str(favorite_list.support_business_apply_start_ymd).split(" ")[0]
#         temp["id"] = favorite_list.id
#         temp["support_business_poster"] = favorite_list.support_business_poster
#
#         temp["support_business_name"] = favorite_list.support_business_name
#         temp["support_business_apply_end_ymdt"] = favorite_list.support_business_apply_end_ymdt
#         temp["support_business_short_desc"] = favorite_list.support_business_short_desc
#         temp["tag"] = []
#         for f in favorite_list.selected_support_business_filter_list.all():
#             temp["tag"].append(f.filter_name)
#         temp["support_business_poster"] = favorite_list.support_business_poster
#         temp["id"] = favorite_list.id
#         result["favorite"].append(copy.deepcopy(temp))
#     #작성중인 지원서
#     result["writing"]=[]
#     print(timezone.now())
...
#         if ap.support_business.support_business_recruit_size == "" or ap.support_business.support_business_recruit_size == "0":
#             temp["comp"] = str(
#                 len(Appliance.objects.all().filter(support_business=favorite_list).filter(is_submit=True))) + " : 1"
#         else:
#             # temp["comp"] = len(Appliance.objects.all().filter(support_business=ap.support_business)) / int(ap.support_business.recruit_size)
#             number = str( round((len(Appliance.objects.all().filter(support_business=favorite_list).filter(
#                 is_submit=True))) / favorite_list.support_business_recruit_size, 1) )
#             if number == "0.0":
#                 number ="0"
#             temp["comp"] = number + " : 1"
#         temp["date"] = str(ap.support_business.support_business_pro_0_end_ymd).split(" ")[0]
...
#
#
# def get_mng_list_in_static(request):
#     add = AdditionalUserInfo.objects.get(id=request.GET.get("gca_id"))
#     mng_list = []
#     for a in add.additionaluserinfo_set.all():
#         support_business=[]
#         for support_business in SupportBusiness.objects.all().filter(support_business_author=a):
#             support_business.append({
#                 "name": support_business.support_business_name,
#                 "id":  support_business.id
#             })
#         mng_list.append(
#             {
#                 "mng_name": a.mng_name,
...
#             }
#         )
#     return JsonResponse(mng_list, safe=False)
#
#
#
# import requests
...
#
#     st = StatTable.objects.all().filter(stat_user_id=my_id).filter(stat_name=my_stat_page).order_by("-stat_timestamp")[0]
#
#
#
#     # if  my_stat_page=="2":
...
#         FavoriteLog.objects.get(user_id=user_id, path_id=path.id).delete()
#         result="remove"
#     else:
...
#         FavoriteLog.objects.get(user_id=user_id, course_id=course.id).delete()
#         result="remove"
#     else:
...
#         FavoriteLog.objects.get(user_id=user_id, clip_id=clip.id).delete()
#         result="remove"
#     else:
...
#
# @csrf_exempt
# def get_favorite_channel_content(request):
#     ad = AdditionalUserInfo.objects.get(id=request.GET.get("gca_id"))
#     result={}
#     fav_clip_list = []
#     for item in ad.favorite_clip.all():
#         fav_clip_list.append(item.id)
#     fav_course_list = []
#     for item in ad.favorite_course.all():
#         fav_course_list.append(item.id)
#     fav_path_list = []
#     for item in ad.favorite_path.all():
#         fav_path_list.append(item.id)
#
#     result["fav_clip"] = fav_clip_list
#     result["fav_course"] = fav_course_list
#     result["fav_path"] = fav_path_list
#
#     return JsonResponse(result, safe=False)
#
#
...
# # #         temp={}
# # #         temp["date"] = fd["date"]
# # #         print(len(Favoritelog.objects.filter(Course=course).filter(date = fd["date"])))
...
#     result["course_play"] = clip.clip_play
#     try:
